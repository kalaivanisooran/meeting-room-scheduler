...
public interface BookingRepo extends CrudRepository<Booking, Long>, JpaSpecificationExecutor<Booking> {
    ...
    @Query("SELECT B FROM Booking B WHERE B.roomInfo.id=:roomId AND B.bookingStartDate=:bookDate   "
    ...
    List<Booking> doBookingSlotAvailable(@Param("bookDate") LocalDate bookDate,

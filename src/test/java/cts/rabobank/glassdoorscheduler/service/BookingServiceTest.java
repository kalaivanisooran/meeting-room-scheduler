...
public class BookingServiceTest {
	...
	@Mock
    MeetingTypeService meetingService;
	...
	public void testHandleExceptionWhileMeetingRoomSchedule() {
		...
		MeetingRoomBookingException exceptionMessage = Assertions.assertThrows(MeetingRoomBookingException.class,
				() -> {
					bookingService.bookRoom(bookingInfo);
				});
		Assertions.assertEquals("Something went wrong while inserting the data into database",
	...
	public void testHandleExceptionWhenCustomMeetingRoomBooking() {
		...
		InvalidInputRequestException exceptionMessage = Assertions.assertThrows(InvalidInputRequestException.class,
				() -> {
					bookingService.customMeetingRoomBooking(this.getRoomDetails(), this.getUserInfo(), this.getMeetingType(), bookingInfo);
				});
	...
	public void testCustomMeetingRoomBooking() {
		bookingInfo.setBookingMode("custom");
	...
	public void testHandleExceptionInCustomMeetingRoomBooking() {
		...
		Assertions.assertThrows(MeetingRoomBookingException.class, () -> {
			bookingService.customMeetingRoomBooking(this.getRoomDetails(), this.getUserInfo(), this.getMeetingType(), bookingInfo);
		});
	}
	...
	public void testCancelMeetingRoomWhichIsNotListed() {
		doThrow(new InvalidInputRequestException("Invalid. Requested meeting room information is not available"))
				.when(bookingRepo).deleteById(any());
	...
	private MeetingType getMeetingType() {
    	return new MeetingType(1L, "Standup");
	}
}